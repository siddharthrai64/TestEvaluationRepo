...
const Head: FC = () => {
    ...
    const searchCache: SearchCache = useSelector((store: any) => store.search);
    ...
    */
    ...
    return (
        <div className="grid grid-flow-col p-5 shadow-lg fixed top-0  w-full bg-white">
            <div className="flex col-span-1">
                <img
                    onClick={() => toggleMenuHandler()}
                    className="cursor-pointer h-10 mx-2"
                ...
                <a href="/">
                    <img
                        className="h-10"
                        src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Logo_of_YouTube_%282015-2017%29.svg/1004px-Logo_of_YouTube_%282015-2017%29.svg.png?20200109235614"
                        alt="youtube-logo"
                    />
                </a>
            </div>
            <div className="col-span-10 px-10">
                <div>
                    ...
                    />
                    <button className="border border-gray-400 p-2 rounded-e-full">
                        Search
                    </button>
                </div>
                {showSuggestions && <div className="fixed bg-white py-2 px-5 w-[37rem] border border-gray-100">
                    <ul>
                        {suggestions.map((suggestion) => {
                            return <li key={suggestion} className="px-4 py-2 shadow-sm hover:bg-gray-100"> 🔎 {suggestion}</li>
                        })}
                    </ul>
                </div>}
            </div>
